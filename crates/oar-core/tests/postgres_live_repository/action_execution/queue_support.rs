...
pub(crate) fn execution_payload(objective_id: &str, kr_id: &str, progress_delta: i64) -> Value {
    json!({
        "target": {
...
pub(crate) struct ExecutionQueueSeed<'a> {
    pub(crate) tenant_id: &'a str,
    pub(crate) user_id: &'a str,
    pub(crate) proposed_action_id: &'a str,
    pub(crate) confirmed_action_id: &'a str,
    pub(crate) idempotency_key: &'a str,
    pub(crate) operation_id: &'a str,
    pub(crate) decision: ProposedActionDecision,
    pub(crate) suggested_payload: Value,
}

pub(crate) async fn seed_confirmed_execution_queue_item(
    pool: &PgPool,
