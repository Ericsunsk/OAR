...
fn postgres_live_operational_recovery_resume_paused_auth_refresh_is_confirmed_and_audited() {
    run_live_postgres_test(
        ...
        |pool| async move {
            ...
            assert_eq!(
                report.resumed_token_grant_id.as_deref(),
                Some("grant_ops_resume_config")
            );
            ...
            assert_eq!(duplicate.resumed_token_grant_id, None);
...
fn postgres_live_operational_recovery_resume_paused_auth_refresh_stale_guard_fails_closed() {
    run_live_postgres_test(
        ...
        |pool| async move {
            ...
            assert_eq!(report.resumed_token_grant_id, None);
            assert_eq!(report.events.len(), 3);
...
async fn token_grant_updated_at_ms(
    pool: &PgPool,
    tenant_id: &str,
    grant_id: &str,
...
async fn audit_event_operation_count(
    pool: &PgPool,
    tenant_id: &str,
    trace_id: &str,
    operation_id: &str,
...
async fn audit_outbox_count_for_trace(
    pool: &PgPool,
    tenant_id: &str,
    trace_id: &str,
) -> Result<i64, sqlx::Error> {
