...
pub(super) fn validate_audit_outbox_payload(payload: &Value) -> PgRepositoryResult<()> {
    let object = payload
        .as_object()
        .ok_or(PostgresRepositoryError::UnsafeAuditOutboxPayload)?;
    if object.is_empty() {
        return Err(PostgresRepositoryError::UnsafeAuditOutboxPayload);
    }
    for (key, value) in object {
        if !is_allowed_audit_outbox_payload_key(key) {
            return Err(PostgresRepositoryError::UnsafeAuditOutboxPayload);
        }
        validate_audit_outbox_payload_entry(key, value)?;
    }
    Ok(())
}

fn is_allowed_audit_outbox_payload_key(key: &str) -> bool {
    matches!(
        key,
        "event_id" | "trace_id" | "event_type" | "sequence" | "tenant_id" | "kind"
    )
}
...
fn validate_audit_outbox_payload_value(value: &Value) -> PgRepositoryResult<()> {
    match value {
