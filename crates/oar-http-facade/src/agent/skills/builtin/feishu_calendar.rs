...
pub(in crate::agent::skills) const SAFETY: &str = "Skill 只描述领域能力；真实读取必须由后端 tool runtime 校验 OAuth scope 后通过 Lark adapter 执行。只读忙闲与受限日程摘要工具可自动执行，建会、邀请、通知等写操作必须 dry-run 和人工确认。";
