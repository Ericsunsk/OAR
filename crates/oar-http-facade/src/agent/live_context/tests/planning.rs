...
async fn live_context_degrades_when_feishu_persistence_is_unavailable() {
    ...
    assert_eq!(request.context.live_feishu_read_statuses.len(), 1);
    assert_eq!(
...
async fn live_context_plans_read_only_tool_when_okr_intent_has_no_evidence_refs() {
    ...
    assert_eq!(request.context.activated_skill_statuses[0].id, "feishu.okr");
    assert_eq!(request.context.live_feishu_read_summaries.len(), 1);
...
#[tokio::test]
async fn live_context_plans_read_only_tool_when_minutes_intent_has_no_evidence_refs() {
    let mut request = live_context_request(
        "查下我的飞书妙记",
        "妙记查询",
        "用户请求实时读取",
    ...
    assert_eq!(request.context.activated_skill_summaries.len(), 1);
    assert!(request.context.activated_skill_summaries[0].contains("feishu.minutes"));
