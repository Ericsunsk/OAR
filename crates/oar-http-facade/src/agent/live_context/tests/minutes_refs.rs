...
#[tokio::test]
async fn minutes_live_context_uses_meeting_refs_before_safe_degrade() {
...
#[tokio::test]
async fn live_context_degrades_invalid_minutes_refs_without_auth_or_raw_echo() {
    let refs = vec![AgentEvidenceRefDTO {
        source_type: "meeting".to_string(),
        source_ref: "minutes://sk-secret-token".to_string(),
    ...
    assert!(summaries[0].contains("source_ref 不是可识别的妙记引用"));
