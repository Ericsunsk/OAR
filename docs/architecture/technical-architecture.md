...
- scheduler 侧已补齐 durable Postgres lease primitive，作为 sweep 执行前的租约门禁；未拿到租约时不得执行 refresh sweep。
...
- scheduled sweep 需要使用 `limit + 1` 作为 backlog 探针：只处理前 `limit` 个 grant，额外候选仅用于 `has_more`，不得分配审计 sequence 或触发 adapter/UoW；当 `has_more` 为真时，下一次运行应使用短 `backlog_next_run_delay_ms` 而不是常规长周期，避免大租户剩余 due grants 被饿住。
...
- core 只暴露“单次租户维护 tick（one-shot tenant maintenance tick）”能力，供上层 runtime 显式触发；该 tick 是后台常驻调度接入前的前置契约。
...
- runtime 层后续可用 Tokio `interval` + cancellation token（或等价机制）驱动周期执行，但循环调度语义属于 runtime，不下沉到 core。
- 安全边界保持不变：refresh 仅经 `AuthAdapter`；不得记录或暴露 raw token、raw CLI stdout/stderr；OKR 写回仍只允许 `ConfirmedAction -> OperationLedger -> LarkAdapter -> AuditEvent`。
- 当前真实 `AuthAdapter` transport、常驻 scheduler、runtime cancellation/retry 策略与生产监控闭环仍未完成，本切片不构成生产无人值守闭环声明。
...
- `PostgresTokenRefreshSweep` 当前仅提供显式触发的 `run_once` handler；虽已具备 lease gate 与 recurring `pending/running` 状态持久化基础，但尚无 daemon/poll loop/HTTP/gRPC runtime 驱动。
- Auth refresh 已补齐 core-only safe transport client boundary：core 可消费上层 runtime transport 返回的 raw envelope，但 raw 内容只在边界内做大小检查与 parser fail-closed，不进入 domain/debug/audit；具体 Rust OpenAPI transport 与授权材料解密提供器已在 adapter crate 完成 fake HTTP 装配验证，真实 Feishu 网络调用和后台 scheduler/daemon 尚未连通，不构成生产 refresh 闭环。
...
- 该边界只解决“可安全消费 runtime transport 输出”；当前 Rust 原生 OpenAPI transport 已完成 fake HTTP 装配验证，真实 Feishu 网络验证、重试节奏和调度生命周期仍待后续接入后台 scheduler。`lark-cli` 只作为 fixture/验证工具，不作为生产主通道。
