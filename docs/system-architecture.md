...
- 当前 facade 已完成真实 `AuthAdapter` production 装配、runtime tick factory production wiring、webhook outbox sink 装配，以及 `/readyz` stage-level 安全聚合观测；core/storage 已新增 operational recovery report、runbook、confirmed 单 grant paused auth refresh resume 和 confirmed failed audit outbox requeue。retry/backoff 策略、crash recovery、admin/operator recovery route 和生产监控闭环仍未完成，本切片不构成生产无人值守闭环声明。
