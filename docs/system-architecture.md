...
- runtime tick future、auth refresh async adapter、Feishu auth transport 和 audit outbox dispatcher 已收敛到 `Send` future 边界，为后续多线程 Tokio 调度/租户 registry 留出装配空间。
...
- 当前真实 `AuthAdapter` production 装配、多租户 runtime registry、retry/backoff 策略与生产监控闭环仍未完成，本切片不构成生产无人值守闭环声明。
