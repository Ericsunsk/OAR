...
- scheduler 侧已补齐 durable Postgres lease primitive，作为 sweep 执行前的租约门禁；并新增 insert-if-missing bootstrap primitive，用于创建缺失的 recurring job 且不重置已有 `pending` / `running` schedule；未拿到租约时不得执行 refresh sweep。
