...
| Lark 集成 | LarkAdapter 优先；OKR 路径已验证可走 Lark CLI，保留 OpenAPI 兜底路径 | Lark CLI 适合智能体工具层，但生产必须可替换 |
...
| Lark CLI 可作为智能体工具层 | 已初步验证 | 方向成立；生产必须锁定版本、做 schema 快照和回归验证 |
...
> 阶段 0.6 已从“纯骨架”进入“过渡态验证”：token refresh service、Postgres UoW、audit 写入与 `run_once` 幂等链路已有部分实测，但生产闭环尚未完成，需要继续打通真实 AuthAdapter/client、后台调度和多端一致性。
...
1. 把 `TokenGrant` refresh 与撤销链路从验证态推进到可持续运行，补齐真实 `AuthAdapter` / client 调用路径与失败恢复。
2. 继续验证 Postgres UoW + `OperationLedger` + `run_once` 的执行一致性，覆盖重试、超时和重复提交场景。
