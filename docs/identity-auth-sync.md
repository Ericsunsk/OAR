...
5. 候选筛选 + 单次 sweep（已部分通过）：已在候选选择与编排链路之间补充“显式触发的一次性 `run_once` sweep”，并加入 `DATABASE_URL`-gated Postgres live tests 覆盖逐 grant 调用既有 orchestrator/Recorder/audit、顺序审计和 `limit = 0` 短路；同时已补齐 durable Postgres scheduler lease primitive 作为 sweep 执行门禁，scheduled sweep 会用 insert-if-missing bootstrap 缺失 recurring job 且不重置已有 `pending` / `running` schedule。该能力不是 daemon，也不是无人值守后台循环。
...
5. 在 tenant maintenance one-shot tick 契约上继续扩展 runtime 装配：最小 `oar-runtime` 已定义 Tokio interval + cancellation 语义，并已补齐多租户 registry、tenant discovery / registry builder 与 repository-backed active tenant discovery 前置边界；facade 启用门禁已要求真实 audit outbox sink，拒绝 noop/local-noop；后续补齐真实 adapter builder、retry/backoff 和生产监控；循环语义继续禁止下沉进 `oar-core`。
