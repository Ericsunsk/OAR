...
当前处于“安全契约已验证、编排回放已打通、Rust OpenAPI HTTP 装配已完成 fake transport 验证、facade daemon 已装配、live Feishu 与生产运维闭环待验证”的过渡状态，不应视为生产 refresh 闭环已完成。
...
4. 具体 Rust OpenAPI transport + scheduler（部分通过）：OAuth refresh 的 reqwest HTTP transport、授权材料解密提供器和生产装配入口已放入 `crates/oar-lark-adapter`，并通过 fake HTTP/transport 验证；facade 已装配 tenant maintenance daemon 触发路径，`/healthz` 保持 liveness，`/readyz` 可观测 daemon lifecycle、runtime round failure 与 `scheduled_sweep` / `outbox_drain` 安全 stage 状态，但这些信号只用于 runtime observability，不等同于 live Feishu 或生产监控闭环验证。真实 Feishu 网络调用、crash recovery 与生产监控仍未闭环，不具备生产就绪声明条件。`lark-cli` 后续只作为本地验证与 fixture 录制工具，不作为生产主通道。
5. 候选筛选 + 单次 sweep（已部分通过）：已在候选选择与编排链路之间补充“显式触发的一次性 `run_once` sweep”，并加入 `DATABASE_URL`-gated Postgres live tests 覆盖逐 grant 调用既有 orchestrator/Recorder/audit、顺序审计和 `limit = 0` 短路；同时已补齐 durable Postgres scheduler lease primitive 作为 sweep 执行门禁，scheduled sweep 会用 insert-if-missing bootstrap 缺失 recurring job 且不重置已有 `pending` / `running` schedule。该 core 能力本身不是 daemon，也不内建无人值守后台循环。
6. tenant maintenance one-shot contract（已部分通过）：`oar-core` 维持 pure core/storage/contracts 边界，不实现 daemon/poll loop/HTTP/gRPC runtime；“显式触发一次的 tenant maintenance tick”可串联 lease-gated refresh scheduled sweep 与 audit outbox drain，并返回两段独立 stage report；scheduled sweep 硬错误不得跳过 outbox drain，stage failure 只暴露安全分类字符串。facade daemon 已负责外层周期触发，但生产闭环仍待 live Feishu、监控和恢复验证。
...
> 身份与 refresh 前置条件成立；生产级 `TokenGrant` 存储、refresh token rotation、幂等执行、后台 maintenance、confirmed paused auth refresh resume 和 confirmed failed outbox requeue 已有 Postgres/Rust 主路径与部分 live DB 验证。当前仍需补齐真实 Feishu live smoke、多端真实联调、crash recovery、告警和运维 route 验证。
...
详细字段设计归入 [`system-architecture.md`](system-architecture.md)。
...
1. `Tenant` / `WorkspaceUser` / `LarkIdentity` Postgres repositories 下一步收敛到变更审计：基础 `tenant_id` 隔离、identity 绑定唯一约束与 typed conflict 已有 live DB 覆盖，仍需补齐 identity 变更 actor / before-after 摘要 / trace 关联。
...
5. 在 tenant maintenance one-shot tick 契约上继续收口 runtime 装配：最小 `oar-runtime` 已定义 Tokio interval + cancellation 语义，并已补齐多租户 registry、tenant discovery / registry builder 与 repository-backed active tenant discovery 前置边界；facade 已用真实 Rust auth adapter builder 与 HTTPS webhook audit outbox sink 装配 daemon，且启用门禁拒绝 noop/local-noop；`/healthz` 与 `/readyz` 仅作为安全聚合运行观测入口，不承载原始平台数据、租户标识或生产完成判定；core/storage 已新增 operational recovery report、confirmed 单 grant paused auth refresh resume 和 confirmed failed audit outbox requeue，用于列出 failed outbox 与 parked grant 的安全恢复项，并在配置修复后恢复单条安全暂停项；后续补齐 retry/backoff、crash recovery、admin/operator recovery route 和生产监控；循环语义继续禁止下沉进 `oar-core`。
...
2. 在已接入的 facade 后台 scheduler/daemon 上补齐无人值守 refresh、失败重试、crash recovery，并把 `/readyz` stage-level 信号接入后续告警验证。
...
- 已在候选筛选与 orchestrator 之间增加单次 refresh sweep（`run_once`）切片：显式触发后对候选逐 grant 调用既有 `PostgresTokenRefreshOrchestrator`，沿用每 grant 的 Recorder/audit 事务语义；sweep 前置为 durable Postgres scheduler lease gate，并持久化 recurring `pending` / `running` 状态；scheduled sweep report 保留 acquisition detail，审计序列按 lease attempt 分配独立 window，降低重试/抢租约后的 `(trace_id, sequence)` 碰撞风险；backlog-aware 调度使用 `limit + 1` 仅探测 `has_more`，只处理前 `limit` 个 grant，剩余候选触发短 `backlog_next_run_delay_ms` 重排；已加入 `DATABASE_URL`-gated live tests 覆盖成功批次、顺序审计、候选过滤继承、`limit = 0` 不调用 adapter 且不写 audit；core sweep 本身不是后台 daemon，不内建无人值守循环能力，也不把整轮 sweep 作为单个跨 grant 大事务。
- 已将“tenant maintenance one-shot tick”明确为 runtime 前置契约：当前仅承诺 core 可被显式触发执行一次租户维护，串联 lease-gated refresh scheduled sweep 与 audit outbox drain，并返回两段独立 stage report；scheduled sweep 硬错误不再跳过 outbox drain，stage failure 只暴露安全分类字符串；`oar-core` 不承诺也不实现 daemon/poll loop/HTTP/gRPC runtime，常驻调度由 facade runtime 层负责。
...
- `Tenant` / `WorkspaceUser` / `LarkIdentity` Postgres repository 基础语义已通过 live DB 验证：覆盖租户隔离、lookup、同 ID 跨租户冲突、同租户 external identity 绑定唯一性与 typed conflict；identity 变更审计字段 / trace 关联仍未补齐。

仍需生产级验证：

- `Tenant` / `WorkspaceUser` / `LarkIdentity` 变更审计可追溯：基础 repository 语义已有 live DB 覆盖，仍需补齐 actor、before-after 摘要和 trace 关联。
- `TokenGrant` Postgres 持久化集成验证：repository 仅处理加密授权包，不接受/返回明文 token。
- `TokenRefreshService` 与 repository command sink 的领域编排已覆盖，refresh audit 事件映射、Postgres roundtrip 和 fake adapter 下的 transactional orchestrator 已验证；facade 后台调度已接入，仍需接入真实 Feishu 网络调用并在真实 adapter 路径下持续验证同一事务边界。
- Auth refresh adapter contract / safe transport 边界已建立：当前已验证“runtime transport raw envelope -> size guard -> safe parser -> `RefreshOutcome` -> decision bridge”链路，并已用 fake HTTP 验证 Rust OpenAPI adapter 装配闭环；adapter crate 侧新增 `DATABASE_URL` gated 集成测试，覆盖 Postgres stored encrypted grant material 进入 Rust OpenAPI auth adapter，再回到 core Postgres CAS/audit 的成功、`20074` config-required、`20037` reauth-required、5xx transient，以及 missing row / stale fingerprint / malformed blob / wrong key material failure 链路，并验证 `refresh_config_required` 会暂停 due-candidate 重试，material failure 不发起 HTTP、不 rotate 且审计脱敏。facade scheduler runtime 已装配，真实 Feishu 网络调用与生产监控仍未闭环。
...
- token refresh background scheduler/daemon 已在 facade 装配启动路径，并已提供 `/readyz` stage-level 安全聚合状态；confirmed 单 grant paused auth refresh resume 与 failed audit outbox requeue 已在 core/storage 接入。当前仍缺真实 Feishu 网络验证、crash recovery、admin/operator recovery route、告警接入和并发/soak 验证，不代表已具备生产无人值守 refresh 闭环。
- Postgres 级 `OperationLedger` 唯一约束 / upsert 的真实数据库验证需在提供 `DATABASE_URL` 的环境持续运行；多进程并发 race 仍需专门压力用例。
- Postgres executor / outbox worker 已通过 facade tenant maintenance daemon 接入真实后台调度与 HTTPS webhook audit outbox sink，并可通过 `/readyz` 观察 outbox drain stage 的安全聚合状态；operational recovery report 已能列出 failed outbox 安全摘要，确认后的 paused auth refresh resume 已能恢复单个 grant，confirmed failed outbox requeue 已能恢复同一 outbox row 的 pending 投递；crash recovery、告警接入和并发/soak 验证仍需补齐。
