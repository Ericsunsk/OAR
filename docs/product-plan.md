...
| 层 | 内容 | 用途 |
| --- | --- | --- |
| 结构记忆 / Ontology Graph | Tenant、User、Team、Objective、KR、Task、Doc、Meeting、Evidence、Action 及关系 | 梳理组织关系和业务对象关系 |
| 语义记忆 / Vector Memory | 文档摘要、会议纪要摘要、任务评论摘要、历史复盘片段 | 从非结构化内容里找相关上下文 |
| 决策记忆 / Decision Memory | 用户确认、编辑、拒绝过的建议及原因；证据可信度反馈；团队偏好 | 学习人如何判断和校准智能体 |

MVP 不必直接上复杂图数据库。第一版可以用 Postgres 实体表 + relation tables 表达结构记忆，用 pgvector 或独立向量库表达语义记忆，用 append-only feedback/event 表表达决策记忆。

记忆必须支持关闭、删除、导出、重建、保留策略和租户隔离。记忆可以影响检索和排序，但不能单独作为写回飞书的依据；任何写回动作仍然必须回到证据链、用户确认和审计记录。
