...
> 完成阶段 0.6 的身份与同步骨架验证：安全保存和刷新 `TokenGrant`，用 `OperationLedger` 保证同一 `ConfirmedAction` 只执行一次，并让 macOS / iOS / 飞书卡片看到一致状态。
...
1. 定义 Rust 后端的 `Tenant`、`OarUser`、`LarkIdentity`、`TokenGrant`、`DeviceSession`、`OperationLedger`、`AuditEvent` schema。
