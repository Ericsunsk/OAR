...
后端 facade 默认本地监听；macOS 客户端的 backend origin 由
`apps/oar/Sources/OAR/App/AppEnvironment.swift` 的 `defaultBackendBaseURL` 决定。当前仓库
为移动端/公网隧道联调临时指向 `https://seven-eels-do.loca.lt`；只做本地开发时可改为
`http://127.0.0.1:8080`，然后启动本地 facade：
...
不会让前端直连模型服务。完整边界见
[`docs/system-architecture.md`](docs/system-architecture.md#7-智能体运行时与模型配置)。
本地验证默认 Agent 需要额外配置：
